						...
						<div class="container text-left" style="padding: 0;">
							<div class="row g-3">
								<div class="col-sm-4">
									...
									<select disabled class="form-select" name="comp" required="required">
										<?php
										$stmt = "SELECT * FROM comp_tb";
										$result = mysqli_query($con, $stmt);
										while ($rows = mysqli_fetch_assoc($result)) {
											$selected = ($rows['comp'] == $row['comp']) ? 'selected' : '';
										?>
											<option <?php echo $selected ?>><?php echo strtoupper($rows['comp']) ?></option>
										<?php } ?>
									</select>
								</div>
						...
						<div class="container text-left" style="padding: 0;">
							<div class="row g-3">
								<div class="col-sm-4">
									...
									<select disabled class="form-select" name="course" required="required">
										<?php
								...
								<div class="col-sm-3">
									<label>Semester<span id="" style="font-size:11px;color:red">*</span> </label>
										...
										<?php $stmt = "SELECT * FROM second_sem WHERE student_id = '$id'";
										$result = mysqli_query($con, $stmt);
										
										if ($result && mysqli_num_rows($result) > 0) {
											$enrolled = "Enrolled in 2nd semester";
										} else {
											$enrolled = "Enrolled in 1st semester";
										} ?>
										<input disabled style="color: black; cursor:not-allowed;" class="form-control" value="<?php echo $enrolled ?>" name="fname" required="required">
								</div>
							</div>
						</div>
